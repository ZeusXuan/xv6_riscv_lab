...
sys_mmap(void)
{
  ...
  // 非法情况:1文件不可读,但是prot设置为可读
  // 2.对文件的修改写回,但是文件此时不可写且prot 为可写
...
sys_munmap(void)
{
  ...
  if(argaddr(0, &addr) < 0 || argint(1, &sz) < 0 || sz == 0)
    return -1;
  ...
  int nunmap = sz - (addr_aligned - addr); // nbytes to unmap , nunmap = addr + sz - addr_aligned
  if(nunmap < 0)
    nunmap = 0;
